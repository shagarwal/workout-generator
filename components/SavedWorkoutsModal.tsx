...
import { X, Trash2, Calendar, Cloud, HardDrive, Clock, FolderOpen } from 'lucide-react';
...
export default function SavedWorkoutsModal({ isOpen, onClose, onLoad }: SavedWorkoutsModalProps) {
  ...
  const loadRecentWorkouts = () => {
    try {
      const recent = localStorage.getItem('recentWorkouts');
      if (recent) {
        setRecentWorkouts(JSON.parse(recent));
      }
    } catch (error) {
            ...
            >
              ...
              {recentWorkouts.length > 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded ${
                  activeTab === 'recent' ? 'bg-gray-900 text-lime-400' : 'bg-gray-700 text-gray-300'
                }`}>
          ...
          {activeTab === 'saved' ? (
            ...
            recentWorkouts.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-sm mt-2">
                  Generate a workout to see it here
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <h3 className="font-bold text-white">{workout.plan.summary.title}</h3>
                        </div>
                        <div className="space-y-1 text-sm">
                          <p className="text-gray-400">
                            <span className="text-lime-400 font-semibold">Muscles:</span> {workout.plan.summary.muscles}
                          ...
                          <div className="flex items-center gap-2 text-gray-500 text-xs mt-2">
                            <Clock size={12} />
                            <span>{new Date(workout.generatedAt).toLocaleString()}</span>
                          </div>
                        </div>
                      </div>
                      <button
